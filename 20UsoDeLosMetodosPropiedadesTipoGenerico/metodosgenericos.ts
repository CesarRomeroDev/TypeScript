 ...
 * Hay varias maneras de hacer esto en función de la variable de tipo. Una forma consiste en declarar un tipo (type) 
 * personalizado como una tupla y, después, extender (extend) la variable de tipo con el tipo personalizado.
 *  En el ejemplo siguiente se declara ValidTypes como una tupla con un valor string y un valor number. Después,
 *  extiende T con el nuevo tipo. De este modo, solo se pueden pasar tipos number o string a la variable de tipo.
 */
 ...
 */
 ...
 /**
  * Observará que TypeScript todavía genera un error con la expresión value + value en la función identity. 
  * Pero ahora sabe que solo los tipos number y string se pueden pasar a la función.
  * Puede usar la restricción de tipos typeof en un bloque if para comprobar el tipo del parámetro value antes 
  * de realizar una operación, tal y como se muestra en el ejemplo siguiente. A partir de la instrucción if, TypeScript 
  * puede determinar si la operación funcionará con los valores proporcionados en el bloque.
  */
  type ValidTypess = string | number;
  function identityy<T extends ValidTypess, U> (value: T, message: U) {   // Return type is inferred
      let result: ValidTypess = '';
  ...
  let numberValue = identityy<number, string>(100, 'Hello');
  let stringValue = identityy<string, string>('100', 'Hello');
